    ...
    def __new__(cls: type, name: str, bases: tuple[type, ...], dct: dict) -> type:
        ...
        for key, val in dct.items():
            if hasattr(val, "__init__") and key != "__init__" and isinstance(val, type):
                if issubclass(val, cls):  # Was collecting enum attribute inits.
...
class configYBase(configBase, metaclass=configMeta):
    ...
    apply_to : list[str | int] | None, optional
        ...
        Names are preferred over indices to avoid ambiguity when the scan structure changes.
    """

    def __init__(self, apply_to: list[str | int] | None = None) -> None:
        # Internal variables
        self._apply_to: list[str | int] | None = apply_to
        """
        The list of scan scan channel names (or indexes) to apply the normalisation to.
    ...
    @apply_to.setter
    def apply_to(self, apply_to: list[str | int] | None):
...
class normMethod(Enum):
    ...
    Allows the user to specify how the normalisation should be performed.
    Values are stored as an enumerate.
    ...
    SUB : 1
        Common for background signals.
        Subtract the background channel from the scan channel.
    DIV : 2
        Common for flux signals. Divide the scan channel by the normalized
        (maximum value=1) background channel amplitude.
    MULT : 3
    """
    ...
    def path(self, path: str | None):
        if os.path.exists(path):
            ...
            if os.path.isfile(path):
                # Automatically set selection to FIXED_SCAN if a file path is given instead of a directory.
                self._channel_selection = self.extSelection.FIXED_SCAN
        else:
...
class configExternalChannel(configChannel, configExternal):
    ...
    def __init__(
        self,
        channel_selection: configExternal.extSelection = extSelection.NONE,
        keyword: str | None = None,
        path: str | None = None,
        norm_method: normMethod = normMethod.NONE,
        channel_name: str | None = None,
...
class configEdges(configYBase):
    ...
    PREEDGE_NORM_TYPE = edgeNormPre
    """The pre-edge normalisation type enumerate."""
    POSTEDGE_NORM_TYPE = edgeNormPost
    ...
    def pre_edge_norm_method(self, method: PREEDGE_NORM_TYPE):
        ...
        if method in configEdges.PREEDGE_NORM_TYPE:
            self._pre_edge_norm_method = method
        else:
            raise ValueError(f"{method} not in {configEdges.PREEDGE_NORM_TYPE}.")
        # If pre-edge not defined, set to default level
        if self.pre_edge_level is None:
            match method:
                case configEdges.PREEDGE_NORM_TYPE.CONSTANT:
                    self.pre_edge_level = self.DEFAULT_PRE_EDGE_LEVEL_CONSTANT
                case configEdges.PREEDGE_NORM_TYPE.LINEAR:
                    self.pre_edge_level = self.DEFAULT_PRE_EDGE_LEVEL_LINEAR
                case configEdges.PREEDGE_NORM_TYPE.EXPONENTIAL:
        ...
        if (
            method is configEdges.PREEDGE_NORM_TYPE.EXPONENTIAL
    ...
    def pre_edge_norm_method(self) -> None:
        self._pre_edge_norm_method = configEdges.PREEDGE_NORM_TYPE.NONE
    ...
    def post_edge_norm_method(self, method: POSTEDGE_NORM_TYPE):
        if method in configEdges.POSTEDGE_NORM_TYPE:
            self._post_edge_norm_method = method
        else:
            raise ValueError(f"{method} not in {configEdges.POSTEDGE_NORM_TYPE}.")

        # Set default if None
        if self._post_edge_level is None:
            match method:
                case configEdges.POSTEDGE_NORM_TYPE.CONSTANT:
                    self._post_edge_level = configEdges.DEFAULT_POST_EDGE_LEVEL_CONSTANT
                case _:
                    # Do nothing if not defined / NONE.
                    pass
    ...
    def post_edge_norm_method(self) -> None:
        self._post_edge_norm_method = configEdges.POSTEDGE_NORM_TYPE.NONE
    ...
    def pre_edge_domain(self, domain: tuple[float, float] | list[int] | None):
        ...
        if (
            self._pre_edge_norm_method is configEdges.PREEDGE_NORM_TYPE.NONE
            and domain is not None
        ):
            self._pre_edge_norm_method = configEdges.PREEDGE_NORM_TYPE.LINEAR
    ...
    def post_edge_domain(self, vals: list[int] | tuple[float, float] | None) -> None:
        ...
        if (
            self._post_edge_norm_method is configEdges.POSTEDGE_NORM_TYPE.NONE
            and vals is not None
        ):
            self._post_edge_norm_method = configEdges.POSTEDGE_NORM_TYPE.CONSTANT
    ...
    def pre_edge_level(self, level: float):
        if self.pre_edge_norm_method is configEdges.PREEDGE_NORM_TYPE.EXPONENTIAL:
            if level <= 0:
                raise ValueError(
    ...
    def pre_edge_level(self):
        if self.pre_edge_norm_method is configEdges.PREEDGE_NORM_TYPE.EXPONENTIAL:
            self._pre_edge_level = configEdges.DEFAULT_PRE_EDGE_LEVEL_EXP
