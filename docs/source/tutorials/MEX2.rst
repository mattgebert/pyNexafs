...
First, locate some data. The example data ``MEX2_13385.mda`` is the spectra of the P3HT sulfonated organic-semiconducting polmyer, measured at the 55 degree `magic` angle. See :ref:`MDA-Files` for more information on processing the MDA file format.
...
``pyNexafs`` hosts a variety of different beamline data formats, which can be accessed through the `parsers` module. Here we use the MEX2 :code:`MEX2_NEXAFS` parser, which is located in the `pyNexafs.parsers.au` submodule.
...
Here we've used a couple of parameters:
